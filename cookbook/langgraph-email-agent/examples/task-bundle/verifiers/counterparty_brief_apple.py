"""Programmatic verifier for the Apple counterparty brief cookbook task."""
...
def verify(run_artifacts: _RunArtifactsLike) -> tuple[bool, str]:
    """Check that the Apple renewal brief includes the expected structure and facts."""
    ...
    fact_checks = {
        "Apple account reference": "apple" in normalized_output,
        "Apple ticker reference": "aapl" in normalized_output,
        "Apple pricing deadline": "friday" in normalized_output
        and "quote" in normalized_output,
        "Apple security blocker": "mobile device management" in normalized_output
        or "soc 2" in normalized_output,
        "Apple legal blocker": "audit rights" in normalized_output
        or "indemnity" in normalized_output,
    ...
    return (
        True,
        "Apple counterparty brief captured the expected renewal facts and company lookup.",
    )
