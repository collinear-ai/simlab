# Integrate An OpenAI Agents SDK Agent With SimLab
...
1. Your OpenAI Agents SDK app code.
2. SimLab `BaseAgent` adapter used as the `--agent-import-path` entrypoint.
3. SDK function tools built from the SimLab `BaseEnvironment`, preferably via the shared adapter module.
...
Read the existing OpenAI Agents SDK entrypoint and determine:

- where the `Agent(...)` is constructed
- how tools are attached
- how the run is executed (`Runner.run()` or `Runner.run_sync()`)
- where the final output is consumed
...
- exposes SimLab tools as SDK function tools
- runs your SDK agent
