...
def verify(run_artifacts: _RunArtifactsLike) -> tuple[bool, str]:
    ...
    fact_checks = {
        ...
        "GlobalParts reference": "globalparts" in normalized
        or "global parts" in normalized,
    ...
    if len(supplier_ids_found) < 2:
        return (
            False,
            f"Report must include at least 2 ERP Supplier IDs (SUP-xxx), found {len(supplier_ids_found)}.",
        )
