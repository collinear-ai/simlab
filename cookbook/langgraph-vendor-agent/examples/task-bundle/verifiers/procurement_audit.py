...
def verify(run_artifacts: _RunArtifactsLike) -> tuple[bool, str]:
    ...
    fin_section = final_output.split("## Remediation Plan")[0].split(
        "## Financial Summary"
    )[-1]
    ...
    if len(dollar_amounts) < 2:
        return (
            False,
            f"Financial Summary must include at least 2 dollar amounts, found {len(dollar_amounts)}.",
        )
    ...
    return (
        True,
        "Procurement audit verified: correct ERP data, cross-referenced findings, and structured remediation.",
    )
