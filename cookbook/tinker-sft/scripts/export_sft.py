...
def convert_artifacts(output_dir: str, out_path: str, min_reward: float = 1.0) -> None:
    """Convert SimLab rollout artifacts to Tinker SFT JSONL."""
    ...
    for rollout_dir in sorted(output.rglob("rollout_*")):
        ...
        for msg in messages:
            ...
            if role == "assistant" and isinstance(content, dict):
                ...
                # The Qwen3InstructRenderer renders message["content"] verbatim, so we
                # serialize tool calls inline using Qwen's *official* format:
                #   <tool_call>{"name": ..., "arguments": {...}}</tool_call>
                # This matches the model's pretraining prior so SFT reinforces rather
                # than fights it, and lets a tolerant eval-side parser score the base
                # model fairly.
                ...
                if tc_blocks:
                    ...
                    serialized = text
                converted.append({"role": "assistant", "content": serialized})
        ...
        if converted:
            task_id = artifacts.get("task_id", rollout_dir.parent.name)
            trajectories.append({"task_id": task_id, "messages": converted})
